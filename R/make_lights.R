...
make_lights <- function(params, df) {
  ...
  params$fed_dates <- seq(
    lubridate::date(experiment_range[1]),
  ...
  light_changes <-
    lapply(params$fed_dates, function(tt) paste(tt, params$lights)) %>%
    unlist()
  #make it datetime
  light_changes <- lubridate::as_datetime(light_changes)
  # subset
  light_changes <-
    light_changes[dplyr::between(
      light_changes,
      experiment_range[1],
      experiment_range[2]
    )]
...
#'  Setting `clip_to_data = FALSE` causes shading to fill the panel (via -Inf/Inf), which can be useful
...
#' @param clip_to_data Logical. If TRUE (default), shading is clipped to the range of the data.
#'   If FALSE, shading extends beyond the data using -Inf/Inf. This can create full-panel shading,
#'   but may cause unexpected visuals if combined with custom `scale_x_datetime()` limits.
...
StatLights <- ggplot2::ggproto(
  ...
  compute_panel = function(
    ...
    extended_dates <- seq(
      lubridate::date(experiment_range[1]) - lubridate::days(1),
      lubridate::date(experiment_range[2]) + lubridate::days(1),
      "1 day"
    )

    # Generate the complete sequence of light changes
    light_on_times <- as.POSIXct(
      glue::glue("{extended_dates} {lights_on_hour}:00:00"),
      tz = time_zone
    )
    light_off_times <- as.POSIXct(
      glue::glue("{extended_dates} {lights_off_hour}:00:00"),
      tz = time_zone
    )
    ...
    light_changes <- light_changes[dplyr::between(
      light_changes,
    ...
    if (
      !length(light_changes) ||
        (lubridate::hour(light_changes[1]) == lights_on_hour &&
    ...
    if (
      !length(light_changes) ||
        (lubridate::hour(tail(light_changes, 1)) == lights_off_hour &&
          lights_on_hour != lights_off_hour)
    ) {
    ...
    if (clip_to_data) {
      df <- df %>%
        dplyr::filter(
          xmin <= experiment_range[2] & xmax >= experiment_range[1]
        ) %>%
        dplyr::mutate(
          xmin = pmax(xmin, experiment_range[1]),
          xmax = pmin(xmax, experiment_range[2])
        )
    } else {
      df <- df %>%
        dplyr::filter(
        ...
        dplyr::mutate(
          xmin = ifelse(
            xmin < experiment_range[1],
            {
              warning(
                "Shading xmin extended to -Inf; check scale_x_datetime() if plot looks off."
              )
              -Inf
            },
            xmin
          ...
          xmax = ifelse(
            xmax > experiment_range[2],
            {
              warning(
                "Shading xmax extended to Inf; check scale_x_datetime() if plot looks off."
              )
              Inf
            },
            xmax
